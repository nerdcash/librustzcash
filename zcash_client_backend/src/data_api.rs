...
/// Information needed to synchronize a transparent address.
#[derive(Debug, Clone, PartialEq, Eq)]
#[cfg(feature = "transparent-inputs")]
pub struct TransparentAddressSyncInfo<AccountId> {
    /// The account that owns the address.
    pub account_id: AccountId,
    /// The transparent address index.
    pub index: u32,
    /// The transparent address to synchronize.
    pub address: TransparentAddress,
    /// The height of the first block to scan for transactions involving this address.
    pub height: BlockHeight,
    /// Whether the address has received an output.
    pub used: bool,
}
...
pub trait WalletRead {
    ...
    /// Gets transparent addresses and their last synchronized block heights for all accounts.
    ///
    /// Use [`WalletWrite::put_latest_scanned_block_for_transparent`] to record the height
    /// after synchronizing an address.
    #[cfg(feature = "transparent-inputs")]
    ...
    >
{
    ...
    ///
    /// Returns an error if the account does not exist or the requested diversifier index is not
