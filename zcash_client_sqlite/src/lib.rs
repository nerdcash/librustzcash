...
pub struct FsBlockDb {
    conn: Connection,
    compact_block_storage: Box<dyn chain::CompactBlockStorage>,
}
...
impl FsBlockDb {
    ...
    pub fn for_path<P: AsRef<Path>>(fsblockdb_root: P) -> Result<Self, FsBlockDbError> {
        let storage = Box::new(chain::DefaultCompactBlockStorage::new(
            fsblockdb_root.as_ref().join("blocks"),
        )?);
        FsBlockDb::for_path_storage(fsblockdb_root, storage)
    }
    ...
    /// that the resulting metadata database is properly initialized and has had all required
    /// migrations applied before use.
    pub fn for_path_storage<P: AsRef<Path>>(
        fsblockdb_root: P,
        compact_block_storage: Box<dyn chain::CompactBlockStorage>,
    ...
    ///
    /// This will return an error if the compact block content corresponding to one of these metadata records
    /// is absent.
    pub fn write_block_metadata(&self, block_meta: &[BlockMeta]) -> Result<(), FsBlockDbError> {
        for m in block_meta {
            self.compact_block_storage.verify_compact_block_stored(m)?;
        }
...
impl std::fmt::Display for FsBlockDbError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            FsBlockDbError::MissingBlock(block_meta) => {
                write!(
                    f,
