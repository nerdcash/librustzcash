...
pub(crate) fn get_transparent_addresses_and_sync_heights<P: consensus::Parameters>(
    conn: &rusqlite::Connection,
    params: &P,
) -> Result<Vec<TransparentAddressSyncInfo<AccountUuid>>, SqliteClientError> {
    let mut stmt = conn.prepare(
        "SELECT
            cached_transparent_receiver_address,
            diversifier_index_be,
            ...
            (SELECT COUNT(*) FROM transparent_received_outputs txo
             WHERE txo.address = addresses.cached_transparent_receiver_address) > 0 AS used
         FROM addresses
    ...
    while let Some(row) = rows.next()? {
        let address = TransparentAddress::decode(
            params,
            &row.get::<_, String>("cached_transparent_receiver_address")?,
        )?;

        let mut diversifier_index_bytes: [u8; 11] = row
            .get::<_, Vec<u8>>("diversifier_index_be")?
            .try_into()
            .map_err(|_| {
                SqliteClientError::CorruptedData(
        ...
        diversifier_index_bytes.reverse();
        let index =
            u32::try_from(DiversifierIndex::from(diversifier_index_bytes)).map_err(|_| {
                SqliteClientError::CorruptedData(
                    "Address index is not within the transparent range.".to_owned(),
                )
            })?;

        result.push(TransparentAddressSyncInfo {
            address,
            index,
            height: BlockHeight::from(row.get::<_, u32>("last_downloaded_transparent_block")?),
            account_id: AccountUuid(row.get("account_uuid")?),
            used: row.get("used")?,
        });
    }
...
pub(crate) fn put_latest_scanned_block_for_transparent<P: consensus::Parameters>(
    ...
    conn.execute(
        "UPDATE addresses
         SET last_downloaded_transparent_block = :block_height
